            ...
            , uint32_t time1, uint32_t time2, uint_fast8_t fifo)
{
    ...
    // UART1_Printf("lis2dw_status oid=%d clock=%u query_ticks=%u next_sequence=%hu"
    //       " buffered=%c fifo=%c limit_count=%hu\r\n"
    //       , oid, time1, time2-time1, ax->sequence
    //       , ax->data_count, fifo, ax->limit_count);
}
...
lis2dw_query(struct lis2dw *ax, uint8_t oid)
{
    ...
    // uint8_t status[2] = {0x27 | LIS_AM_READ , 0};
    // spidev_transfer(ax->spi, 1, sizeof(status), status);
    // UART1_Printf("status %02x %02x\r\n",status[0],status[1]);
    // uint16_t rx = (((d[1] << 8) | d[0])) - ((d[1] & 0x80) << 9);
    // uint16_t ry = (((d[3] << 8) | d[2])) - ((d[3] & 0x80) << 9);
    // uint16_t rz = (((d[5] << 8) | d[4])) - ((d[5] & 0x80) << 9); 
    // UART1_Printf("lis2dw_query : %d %d %d\r\n",rx,ry,rz);
    // UART1_Printf("%02x %02x %02x %02x %02x %02x\r\n",d[0],d[1],d[2],d[3],d[4],d[5]);

    ax->data_count += 6;
    if (ax->data_count + 6 > ARRAY_SIZE(ax->data))//48字节
    ...
    if (!fifo_empty&&!(ax->fifo_disable)) {
        ...
        sched_wake_task(&lis2dw_wake);
        // UART1_Printf("lis2dw_query : wake\r\n");
    } else if (ax->flags & LIS_RUNNING) {
        // Sleep until next check time
        sched_del_timer(&ax->timer);
        ax->flags &= ~LIS_PENDING;
        // UART1_Printf("lis2dw_query : sleep\r\n");
