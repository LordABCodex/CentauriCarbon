...
uint_fast8_t
command_encode_and_frame(uint8_t *buf, const struct command_encoder *ce, va_list args)
{
...
void command_sendf(const struct command_encoder *ce, ...)
{
    if (readb(&in_sendf))
        ...
        return;
    writeb(&in_sendf, 1);
    // if (ce->msg_id!= 69&& ce->msg_id!=70 && ce->msg_id!=0&& ce->msg_id!=71&& ce->msg_id!=72&& ce->msg_id!=101) {
    //     UART1_Printf("msgid : %d size : %d num_params : %d\r\n", ce->msg_id, ce->max_size, ce->num_params);
    // }
    va_list args;
    va_start(args, ce);
...
int_fast8_t
command_find_and_dispatch(uint8_t *buf, uint_fast8_t buf_len, uint_fast8_t *pop_count)
{
