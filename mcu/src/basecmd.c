...
{
    if (oid >= oid_count){
        UART1_Printf("Invalid oid:%d\r\n",oid);
        UART1_Printf("Invalid oid, oid:%d type:%x\r\n",oid,type);
        shutdown("Invalid oid:\r\n");
    }else if (type != oids[oid].type) {
        UART1_Printf("Invalid oid type, oid:%d type:%x\r\n",oid,type);
...
{
    UART1_Printf("\r\nconfig is_config=%d crc=%u move_count=%hu is_shutdown=%d\r\n", is_finalized(), config_crc, move_count, sched_is_shutdown());
    sendf("config is_config=%c crc=%u move_count=%hu is_shutdown=%c", is_finalized(), config_crc, move_count, sched_is_shutdown());
}
...
void command_finalize_config(uint32_t *args)
{
    move_finalize();
    config_crc = args[0];
    UART1_Printf("command_finalize_config crc = %u\r\n", config_crc);
}
...
void command_identify(uint32_t *args)
{
    ...
    if (offset >= isize)
        ...
        count = isize - offset;
    sendf("identify_response offset=%u data=%.*s", offset, count, &command_identify_data[offset]);
}
